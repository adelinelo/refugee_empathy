...
Below for robustness check with logistic regression:
```{r,echo=FALSE}
#logistic
library(rms)
fit=lrm(y4 ~ Empathy+sex+Empathy*sex+birthyr+Empathy*birthyr+immigrant2+Empathy*immigrant2+educ+Empathy*educ+religpew+Empathy*religpew + partyid3_names + Empathy*partyid3_names +ethnocentric1 + Empathy*ethnocentric1, x=T, y=T, data=ControlEmpathy.data.w1)
robcov(fit, cluster=ControlEmpathy.data.w1$caseid) 
#bootcov(fit, cluster=ControlEmpathy.data.w1$caseid) 
...
```{r,echo=FALSE}
#logistic,#religpew offending, remove
library(rms)
fit=lrm(y4 ~ Persuasion+sex+Persuasion*sex+birthyr+Persuasion*birthyr+immigrant2+Persuasion*immigrant2+educ+Persuasion*educ#+religpew+Persuasion*religpew 
        + partyid3_names + Persuasion*partyid3_names +ethnocentric1 + Persuasion*ethnocentric1, x=T, y=T, data=ControlPersuasion.data.w1)
robcov(fit, cluster=ControlPersuasion.data.w1$caseid) 
#bootcov(fit, cluster=ControlPersuasion.data.w1$caseid) 
...
```{r}
#average refugee rating for those who wrote a letter (group 1) and those who did not (group 0): all controls
t.test(y2~Y6_new,data=Control.data.y2)#use Y6 for
  #just wave 1
t.test(y2~Y6_new,data=Control.data.y2.w1)
#average refugee rating for those who wrote a letter (group 1) and those who did not (group 0): only Dems
t.test(y2~Y6_new,data=subset(Control.data.y2,partyid3_names=="Democrat"))
  #just wave 1
t.test(y2~Y6_new,data=subset(Control.data.y2.w1,partyid3_names=="Democrat"))
#average refugee rating for those who wrote a letter (group 1) and those who did not (group 0): only Independents
t.test(y2~Y6_new,data=subset(Control.data.y2,partyid3_names=="Independent"))
  #just wave 1
t.test(y2~Y6_new,data=subset(Control.data.y2.w1,partyid3_names=="Independent"))
#average refugee rating for those who wrote a letter (group 1) and those who did not (group 0): only Republicans
t.test(y2~Y6_new,data=subset(Control.data.y2,partyid3_names=="Republican"))
  #just wave 1
t.test(y2~Y6_new,data=subset(Control.data.y2.w1,partyid3_names=="Republican"))
```


# ACDE for nudge hypothesis: are people who are most likely to respond to treatment with writing letters also maxed out on ratings values?

```{r}
install_github("mattblackwell/DirectEffects", ref = "master")
library(DirectEffects)
# Creating the mediator
ControlEmpathy.data.y2.w1$y2binary<-ifelse(ControlEmpathy.data.y2.w1$y2==7|ControlEmpathy.data.y2.w1$y2==6,1,0)
first<-lm(Y6_new ~ Empathy + y2binary,data=ControlEmpathy.data.y2.w1)
Y6_new.mediator.predicted <- ControlEmpathy.data.y2.w1$y2binary * coef(first)[3]
Y6_new.mediator.adjusted<-ControlEmpathy.data.y2.w1$Y6_new-Y6_new.mediator.predicted
ControlEmpathy.data.y2.w1.version_1<-cbind(ControlEmpathy.data.y2.w1,Y6_new.mediator.adjusted)
# Subset for low
ControlEmpathy.data.y2.w1.version_1.low_rating<-ControlEmpathy.data.y2.w1.version_1[which(ControlEmpathy.data.y2.w1.version_1$y2binary==0),]
# Subset for high
ControlEmpathy.data.y2.w1.version_1.high_rating<-ControlEmpathy.data.y2.w1.version_1[which(ControlEmpathy.data.y2.w1.version_1$y2binary==1),]
low_rating.predicted<-lm(Y6_new.mediator.adjusted ~ Empathy,data=ControlEmpathy.data.y2.w1.version_1.low_rating)
summary(low_rating.predicted)
high_rating.predicted<-lm(Y6_new.mediator.adjusted ~ Empathy,data=ControlEmpathy.data.y2.w1.version_1.high_rating)
summary(high_rating.predicted)
#bootstrapped s.e. for coefficient on Empathy
#get Beta fn
...
results_low <- boot(data=ControlEmpathy.data.y2.w1.version_1.low_rating, statistic=beta, R=10000, formula=Y6_new.mediator.adjusted ~ Empathy +  sex)
results_high <- boot(data=ControlEmpathy.data.y2.w1.version_1.high_rating, statistic=beta, R=10000, formula=Y6_new.mediator.adjusted ~ Empathy +  sex)
```
